    ...
    let rows: Vec<(i64,)> = sqlx::query_as(&format!(
        "SELECT d.id FROM devices d
          WHERE d.status = 'up' AND (
                (EXISTS (SELECT 1 FROM device_credentials dc WHERE dc.device_id = d.id)
                 AND (d.inventory_at IS NULL OR d.inventory_at < now() - make_interval(secs => $1)))
             OR (NOT EXISTS (SELECT 1 FROM device_credentials dc WHERE dc.device_id = d.id)
...
async fn load_credentials(state: &AppState, device_id: i64) -> Result<Vec<Credential>> {
...
pub async fn poll_device(state: &AppState, device_id: i64) -> Result<()> {
    let row: Option<DeviceRow> = sqlx::query_as(
        "SELECT host(ip) AS ip, open_ports, ssh_host_key, inventory, inventory_error,
                COALESCE(name, hostname, host(ip)) AS label
           FROM devices WHERE id = $1",
    )
    .bind(device_id)
    .fetch_optional(&state.db)
    .await?;
    let Some(DeviceRow { ip, open_ports, ssh_host_key: host_key, inventory: previous, inventory_error: previous_error, label }) =
        row
    else {
        return Ok(());
    };
    ...
    if inventory.is_empty() {
        ...
        let error = if has_linked { errors.join(" · ") } else { String::new() };
    ...
    let stats = derive_stats(&inventory, previous.as_ref());
    ...
    if stats.iter().any(Option::is_some) {
        sqlx::query(
            "INSERT INTO device_stats (time, device_id, cpu_pct, mem_pct, disk_pct, temp_c, rx_bps, tx_bps)
...
fn derive_stats(inv: &Value, previous: Option<&Value>) -> [Option<f64>; 6] {
    let ssh = &inv["ssh"];
    let snmp = &inv["snmp"];
    ...
    let temp = ssh["temp_c"].as_f64().or_else(|| snmp["synology"]["temp_c"].as_f64());

    // Datenrate aus der Differenz der Byte-Zähler zur vorherigen Abfrage
    ...
    if let Some(prev) = previous {
        ...
        if let Some(seconds) = seconds.filter(|s| *s > 10.0 && *s < 7200.0) {
            let source = if ssh.is_object() { "ssh" } else { "snmp" };
            let rate = |field: &str| -> Option<f64> {
                let now = inv[source]["interfaces"].as_array()?;
                let before = prev[source]["interfaces"].as_array()?;
                let mut sum = 0.0;
                for iface in now {
                    // Loopback (Typ 24) und virtuelle Schnittstellen nicht mitzählen
                    if iface["type"].as_f64() == Some(24.0) {
                        continue;
                    }
                    let old = before.iter().find(|b| b["name"] == iface["name"]);
                    if let (Some(n), Some(o)) = (iface[field].as_f64(), old.and_then(|o| o[field].as_f64())) {
                        if n >= o {
                            sum += n - o;
                        }
                    }
                }
                Some(sum * 8.0 / seconds)
            };
            rx = rate("rx_bytes");
            tx = rate("tx_bytes");
        }
    }
    [cpu, mem, disk, temp, rx, tx]
}
...
mod tests {
    ...
    fn datenrate_aus_zaehlerdifferenz() {
        ...
        let s = derive_stats(&now, Some(&prev));
